...
export default async function RadarViralPage() {
  ...
  let aviso =
    "Prévia com dados de exemplo. Configure YOUTUBE_API_KEY em .env.local pra ver tendências reais.";
  ...
  try {
    ...
    aviso =
      radar.fonte === "cache"
        ? "Dados reais do YouTube (cache de até 12h, pra não estourar a cota da API)."
        : "Dados reais do YouTube, atualizados agora.";
  } catch (err) {
    console.error(
    ...
    <div>
      <header className="mb-6">
        ...
        <p className="mt-1.5 max-w-[68ch] text-sm leading-relaxed text-zinc-500">
          O que está em alta agora, por categoria. Entenda o formato que está
          funcionando antes de gravar o seu — e salve o que servir de
          referência.
        </p>
        <p
          className={
